...
def get_spatial_position_embedding(pos_emb_dim, feat_map, temperature=10000):
    assert pos_emb_dim % 4 == 0, "d_model must be divisible by 4"
    ...
    y = y / (H - 1) * 2 * math.pi  # (H,)
    x = x / (W - 1) * 2 * math.pi  # (W,)
    y_embed = y[:, None].repeat(1, W)  # (H, W)
    x_embed = x[None, :].repeat(H, 1)  # (H, W)

    dim_t = torch.arange(pos_emb_dim // 4, device=feat_map.device, dtype=torch.float32)
    dim_t = temperature ** (2 * (dim_t // 2) / pos_emb_dim)  # (d_model/2,)

    # 3) division and sin/cos
    pos_y = y_embed[:, :, None] / dim_t  # (H, W, d/2)
    pos_x = x_embed[:, :, None] / dim_t

    # 4) stack and flatten
    pos_y = torch.stack([pos_y.sin(), pos_y.cos()], dim=3).flatten(2)  # (H, W, d_model)
    pos_x = torch.stack([pos_x.sin(), pos_x.cos()], dim=3).flatten(2)

    # 5) concat
    pos = torch.cat([pos_y, pos_x], dim=2)        # (H, W, d_model)
    pos = pos.view(-1, pos_emb_dim)               # (H*W, d_model)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, pad_mask=None):
        ...
        # cosine similarity
        dots = torch.einsum('b h w i d, b h w j d -> b h w i j', q, k)  # cos(q,k)
        ...
        if self.relative_pos_embedding:
            ...
            coords_flat = rel_c.view(M2 * M2, 2)  # (M2*M2, 2)
            # cpb_flat: (M2*M2, heads)
            cpb_flat = self.cpb_mlp(coords_flat)  # (M2*M2, heads)
            # reshape to (M2, M2, heads)
            cpb = cpb_flat.view(M2, M2, self.heads)  # (M2, M2, heads)
            cpb = cpb.permute(2, 0, 1)  # (heads, M2, M2)
            dots = dots + cpb.unsqueeze(0).unsqueeze(2)  # broadcast to (b, heads, windows, M2, M2)
        else:
        ...
        if pad_mask is not None:
            if pad_mask.dim() == 2:
                pad_mask_batch = pad_mask.unsqueeze(0).expand(b, -1, -1)  # (B, nw, win^2)
            else:
                pad_mask_batch = pad_mask  # (B, nw, win^2)
            pad_mask_expand = pad_mask_batch.view(b, 1, nw, 1, self.window_size * self.window_size)
            dots = dots + pad_mask_expand.to(dots.device)

        attn = dots.softmax(dim=-1)  # (B, heads, nw, win^2, win^2)
...
def windows_attn_to_global(attn, H_pad, W_pad, window_size, shifted):
    ...
    win = window_size
    assert win * win == win2, "unexpected attn local dim"
    nw_h = H_pad // win
    nw_w = W_pad // win
    ...
    for w_idx in range(nw):
        ...
        # place local window attention into global matrix
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, spatial_position_embedding, feat_hw=None):
        ...
        assert H * W == N, f"Resolved H*W != N ({H}*{W} != {N})"
        ...
        if pad_h > 0 or pad_w > 0:
            x_map = x_map.permute(0, 3, 1, 2)  # (B, D, H, W)
            x_map = F.pad(x_map, (0, pad_w, 0, pad_h))
            x_map = x_map.permute(0, 2, 3, 1)  # (B, H_pad, W_pad, D)

            pos_map = pos_map.permute(2, 0, 1)
            pos_map = F.pad(pos_map, (0, pad_w, 0, pad_h))
            pos_map = pos_map.permute(1, 2, 0)  # (H_pad, W_pad, D)
        ...
        if self.use_abs_pos:
            # broadcast pos_map to batch and add
        ...
        # pass through Swin blocks, passing pad_mask_batch to WindowAttention via blocks
        for blk in self.blocks:
            res = blk(x_map, pad_mask=pad_mask_batch)
            if isinstance(res, tuple):
                x_map, (attn, shifted_flag) = res
                attn_weights_raw.append(attn)  # (B, heads, nw, win^2, win^2)
                ...
                if self.expand_to_global:
                    ...
                    cropped = global_attn[:, :, valid_idx][:, :, :, valid_idx]  # (B, heads, N, N)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, num_heads, d_model, ff_inner_dim,
        ...
        # Multi-Head Self Attention module for all decoder layers
        ...
        # Multi-Head Cross Attention Module for all decoder layers
        ...
        # MLP (FFN) Module for all decoder layers
        ...
        # Norm for Multi-Head Self Attention Module for all decoder layers
        ...
        # Norm for Multi-Head Cross Attention Module for all decoder layers
        ...
        # Norm for MLP (FFN) Module for all decoder layers
        ...
        # Dropout for Multi-Head Self Attention Module for all decoder layers
        ...
        # Dropout for Multi-Head Cross Attention Module for all decoder layers
        ...
        # Dropout for MLP (FFN) Module for all decoder layers
    ...
    def forward(self, query_objects, encoder_output,
        ...
        for i in range(self.num_layers):
            ...
            v = in_attn
            out_attn, _ = self.self_attention_s[i](
                query=q,
                key=k,
                value=v
            )
            ...
            v = encoder_output
            out_attn, decoder_cross_attn = self.cross_attention_s[i](
                query=q,
                key=k,
                value=v
            )
...
class DETR(nn.Module):
    ...
    A forward pass goes through the following layers:
        1. Backbone Call(EfficientNet lite0)
    ...
    def forward(self, x, targets=None, score_thresh=0, use_nms=False):
        ...
        if enc_attn_weights is not None and enc_attn_weights.numel() != 0:
            if enc_attn_weights.dim() == 5:  # (L, B, heads, N, N)
                enc_attn_weights = enc_attn_weights.mean(dim=2)  # (L, B, N, N)
